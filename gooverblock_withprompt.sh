...
BINDIR=${BASEDIR}/bin
TASKDIRS="task1 task2 task3 task4 task5"
...
function task6()
{
	...
	do
		...
		do
			...
			m=`expr $RANDOM % 5`
			let "m++"
			echo "compared with seeding resolution of ${m}x${m}x${m}, bundle of $CURFB" 
			key=1
			if [ $m -ne $n ];then
				key=2
			fi
			...
			${NORMALCASEDATADIR}/region_for_task4/s${n}/${CURFB}/${RESID}/region_s${n}.data \
			-f \
			${NORMALCASEDATADIR}/region_for_task4/s${m}/${CURFB}/${RESID}/region_s${m}.data \
			-s \
			${NORMALCASEDATADIR}/region_for_task4/s${n}/${CURFB}/${RESID}/skeleton_region_s${n}.data \
			-s \
			${NORMALCASEDATADIR}/region_for_task4/s${m}/${CURFB}/${RESID}/skeleton_region_s${m}.data \
			-p ${TASKPROGIDFILE} \
			-r ${TUBERADIUS} \
			-e \
			-k ${key} \
