...
//
// ----------------------------------------------------------------------------
#if !defined(_MITUBERENDER_VIEW_H_)
#define _MITUBERENDER_VIEW_H_
...
class CMitubeRenderView: public CSitubeRender {
public:
	CMitubeRenderView(int argc, char **argv);
	virtual ~CMitubeRenderView();
...
private:
	...
	// a set of fiber indices
	vector<string> m_strfiberIndices;
	// a set of skeletonic geometries
	vector<string> m_strskeletons;
	...
	static CMitubeRenderView* m_pmitInstance;
	...
	// NOTE : SINCE this CMitubeRenderView class acts as a container of a set of
