...
void CSitubeRender::drawCanvas(bool bGrid)
{
	...
	glBegin(GL_POLYGON);
		glVertex3f( -w * fscale, -1, -h*18 * fscale);
...
bool CSitubeRender::_getAnswer(unsigned char key)
{
	...
	*/
	...
	if ( m_curseloptidx == int(m_optPanel.m_buttons.size()) - 1 ) {
		...
		ostrprmpt << "Your answer [" << m_optPanel.m_buttons[m_nTaskAnswer].text << "] is ";
		...
		if ( m_nKey == atoi(m_strAnswer.c_str()) ) {
			...
			m_strAnswer += " (wrong).";
			ostrprmpt << "not quite right, the key is " << m_optPanel.m_buttons[m_nKey-1].text << "!.\n";
		}
		m_cout << "Task completed with Answer : " << m_strAnswer << "\n";
		cleanup();
		exit(EXIT_SUCCESS);
