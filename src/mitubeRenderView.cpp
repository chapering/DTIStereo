	...
	m_bStereo(false)
{
	...
	addOption('i', true, "fiber-indices", "name of file holding fiber indices"
		   ",each one given by a -i");
	addOption('s', true, "skeleton-geometries", "skeleton of the primary geometrical model"
		   ",each one given by a -s ");
}
...
int CMitubeRenderView::handleOptions(int optv) 
{
	switch( optv ) {
		...
		case 'd':
			...
			return 0;
		case 'i':
			m_strfiberIndices.push_back( optarg );
			return 0;
		case 's':
			m_strskeletons.push_back( optarg );
			return 0;
		case 'b':
		case 'l':
		case 'g':
...
int CMitubeRenderView::_assign_argvs(int renderIdx)
{
	...
	int nItmes = m_argc - (int)m_strfnsrcs.size()*2 - (int)m_strdwidirs.size()*2 - (int)m_strfiberIndices.size()*2
						- (int)m_strskeletons.size()*2;
	// -f is compulsory
	nItmes +=2;
	...
	if ( renderIdx < (int)m_strfiberIndices.size() ) {
		nItmes += 2;
	}

	if ( renderIdx < (int)m_strskeletons.size() ) {
		nItmes+=2;
	}

	m_allargvs[renderIdx] = new char*[nItmes];
	int cntf = 0, cntd = 0, cntthis = 0, cnti = 0, cnts = 0;
...
int CMitubeRenderView::_armRenders() 
{
	...
	for (int i = 0; i < nRender; i++) {
		...
		m_renders[i]->switchhelp(nRender - 1 == i);
		m_renders[i]->run();
	}
