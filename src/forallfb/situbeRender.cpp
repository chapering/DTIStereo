	...
	m_strResponse("")
{
	...
	addOption('u', true, "foreground-geometry", "the name of source file"
			" containing foreground geometry and in the format of tgdata");
 ...
 */
void CSitubeRender::buildTubefromLine(unsigned long lineIdx, bool forfb) 
{
	vector<GLfloat> & line = m_loader.getElement( lineIdx );
	if ( forfb ) {
		line = m_fbloader.getElement( lineIdx );
	}
	unsigned long szPts = static_cast<unsigned long>( line.size()/6 );
	...
	tube_colors.resize( szPts * m_lod * 4 );
	...
	for (unsigned long idx1 = 0, idx2 = 1; idx1 < szPts; idx1++) {
		...
		if (forfb || (line [ idx1*6 + 0 ] == line [ idx1*6 + 1 ] &&
				line [ idx1*6 + 2 ] == line[idx1*6+1] ) ) {
			scaleFactor /= 1.5;
		}
		...
		for (GLubyte l = 0; l < m_lod; ++l) {
			...
			tube_colors [ idx1*4 * m_lod + 4*l + 0 ] = line [ idx1*6 + 0 ];
			tube_colors [ idx1*4 * m_lod + 4*l + 1 ] = line [ idx1*6 + 1 ];
			tube_colors [ idx1*4 * m_lod + 4*l + 2 ] = line [ idx1*6 + 2 ];
			tube_colors [ idx1*4 * m_lod + 4*l + 3 ] = 1.0;

			if (forfb || (line [ idx1*6 + 0 ] == line [ idx1*6 + 1 ] &&
				line [ idx1*6 + 2 ] == line[idx1*6+1] )) {
				tube_colors [ idx1*4 * m_lod + 4*l + 3 ] = 0.2;
			}
	...
	if (forfb) {
		m_fbtubevertices[ lineIdx ] = tube_vertices;
		m_fbtubenormals[ lineIdx ] = tube_normals;
		m_fbtubecolors[ lineIdx ] = tube_colors;
		m_fbtubefaceIdxs[ lineIdx ] = tube_faceIdxs;
		m_fbencodedcolors[ lineIdx ] = tube_encodedcolors;
	}
	else {
...
int CSitubeRender::loadGeometry() 
{
	if ( 0 != m_loader.load(m_strfnsrc) ) {
		m_cout << "Loading geometry from " << m_strfnsrc << " failed - GLApp aborted abnormally.\n";
		return -1;
	}

	if ( "" != m_strfnfg && 0 != m_fbloader.load(m_strfnfg) ) {
		m_cout << "Loading geometry from " << m_strfnfg << " failed - GLApp aborted abnormally.\n";
		return -1;
	}
	...
	unsigned long szTotal = m_fbloader.getSize();
	m_fbtubevertices.resize( szTotal );
	m_fbtubenormals.resize( szTotal );
	m_fbtubecolors.resize( szTotal );
	m_fbtubefaceIdxs.resize ( szTotal );
	m_fbencodedcolors.resize( szTotal );
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		buildTubefromLine( idx,true );
	}

	szTotal = m_loader.getSize();
	...
	*/
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		...
		if ( m_bUseDirectionColor ) {
			...
			glColorPointer(4, GL_FLOAT, 0, &m_alltubecolors[idx][0]);
		}
	...
	szTotal = m_fbloader.getSize();
	for (unsigned long idx = 0; idx < szTotal; ++idx) {

		glVertexPointer(3, GL_FLOAT, 0, &m_fbtubevertices[idx][0]);

		if ( m_bUseDirectionColor ) {
			glColorPointer(3, GL_FLOAT, 0, &m_fbencodedcolors[idx][0]);
		}
		else if ( m_bUseOrgColor ) {
			glColorPointer(4, GL_FLOAT, 0, &m_fbtubecolors[idx][0]);
		}

		if ( m_bVnormal ) {
			glNormalPointer(GL_FLOAT, 0 ,&m_fbtubevertices[idx][0]);
		}
		else {
			glNormalPointer(GL_FLOAT, 0 ,&m_fbtubenormals[idx][0]);
		}

		glDrawElements(GL_QUADS, m_fbtubefaceIdxs[idx].size(), 
				GL_UNSIGNED_INT, &m_fbtubefaceIdxs[idx][0]);
...
void CSitubeRender::_drawMarkers()
{
	...
	for ( int k = 0; k < (int)m_boxes.size(); k++ ) {
		...
		*/
		printStrokeText(m_fbnames[k].c_str(), 
				m_tumorBoxMin[k].x, m_tumorBoxMin[k].y, m_tumorBoxMax[k].z, 
				0.0, 1.0, 1.0, 1.0);
	}
