...
void CGLApp::draw()
{
	...
	*/
	...
	/*
	m_stereoinfo.eyesep =  fabs(mvmat[14]) * 1.0 / m_eyeZ;
	if ( m_stereoinfo.eyesep >= 1.0 ) {
		m_stereoinfo.eyesep = 1.0;
	}
	*/
	//cout << "mvmat[14] = " << mvmat[14] << ", eyeZ=" << m_eyeZ << " eyesep= " << m_stereoinfo.eyesep << "\n";
	GLfloat wid, wf = 50.0;
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glFrustum(m_stereoinfo.left, m_stereoinfo.right, -m_stereoinfo.asp, m_stereoinfo.asp, m_near*2.0, m_far);
	//glFrustum(-m_stereoinfo.left+m_stereoinfo.asp, m_stereoinfo.right+m_stereoinfo.asp, m_stereoinfo.bottom, m_stereoinfo.top, m_near, m_far);
	/*
	cout << "left=" << m_stereoinfo.left << ", right=" << m_stereoinfo.right << ", asp=" << m_stereoinfo.asp <<
		", near=" << m_near << ",  far=" << mvmat[14] << "\n";
	*/
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glFrustum(-m_stereoinfo.right, -m_stereoinfo.left, -m_stereoinfo.asp, m_stereoinfo.asp, m_near*2.0, m_far);
	//glFrustum(-m_stereoinfo.left-m_stereoinfo.asp, m_stereoinfo.right-m_stereoinfo.asp, m_stereoinfo.bottom, m_stereoinfo.top, m_near, m_far);
	//
	...
	/// finish rendering stereoscopic geometries
	/*
	glMatrixMode(GL_PROJECTION);
	glPopMatrix();
	*/
...
void CGLApp::onReshape ( GLsizei w, GLsizei h )
{
   ...
   if (m_bStereo) {
		/*
		static GLfloat DTR = 0.0174532925;

		m_stereoinfo.top = m_near * tan ( DTR * m_fvoy/2.0 );
		m_stereoinfo.bottom = - m_stereoinfo.top;

		m_stereoinfo.right = (GLfloat)w / (GLfloat)h * m_stereoinfo.top;
		m_stereoinfo.left = m_stereoinfo.right;
		m_stereoinfo.asp = m_stereoinfo.eyesep/2.0 * m_near / m_maxz; 
		*/

		GLfloat wid, wf = 50.0;
		m_stereoinfo.asp = (GLfloat) h / (GLfloat) w;
		wid = m_stereoinfo.fixpoint * (1.0 / wf);

		m_stereoinfo.left = -wf * ((wid - 0.5 * m_stereoinfo.eyesep) / m_stereoinfo.fixpoint);
		m_stereoinfo.right = wf * ((wid + 0.5 * m_stereoinfo.eyesep) / m_stereoinfo.fixpoint);

		//glTranslatef(0.0, 0.0, -m_stereoinfo.fixpoint);
	} else {
		glMatrixMode  ( GL_PROJECTION ) ;
		glLoadIdentity( ) ;
