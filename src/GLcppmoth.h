...
//				zooming
// @June 4th 2011
//			.When record user's rotating operation, keep track of the rotation
//			axis and angle as well; the same for operation like zooming and
//			translating, with these log data, user exploration process can be,
//			as expected, completely restored.
// @Aug. 27th 2011
//			.Add configurable STEREOSCOPIC display support for display study
//			based on the density study
// @Jun. 13rd 2012
//		- Change to "correct method" for stereo rendering
//			
...
public:
	 ...
	 * @ note :the display of stereo view depends on the both monitor and graphics 
	 *   card supporting this feature although the capability of rendering geometries
	 *   rests only on openGL functionalities
	 * @param bstereo a boolean giving the state into which this function will turn
	 * @param eyesep a GLfloat value giving the eye separation parameter value
	 * @param fixpoint a GLfloat value giving the fixation point distance 
	 * @return none
	 */
	void setStereo(bool bstereo = true, GLfloat eyesep=-1.0, GLfloat fixpoint=-1.0);
	 ...
	 * @return none
	 */
	void setClearColor(GLclampf red=.5, GLclampf green=.5, 
						GLclampf blue=.5, GLclampf alpha=1.0);
	/*
...
protected:
	...
	/*
	 * @brief the down-to-eartch drawing procedure called by draw(). This new
	 * overloadable is added to gear into stereoscopic display at the moment
	 * @return none
	 */
...
protected:
	...
	/* if enable stereoscopic display */
	bool m_bStereo;
	/* parameters concerning stereo rendering */
	stereoinfo_t m_stereoinfo;
	CAMERA camera;
	XYZ origin;
};
