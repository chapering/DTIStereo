...
{	
	...
	*/
    camera.vp.x = 0;
    camera.vp.y = 0;
    camera.vp.z = m_dz*1.5 * 6.0 /scale;
...
void CGLGadget::setVertexCoordRange(GLdouble dx, GLdouble dy, GLdouble dz)
{
	...
	m_dx = 126.266174, m_dy = 124.263704, m_dz = 70.756078;
}

void CGLGadget::setStereo(bool bstereo, GLfloat eyesep, GLfloat fixpoint)
{
	m_bStereo = bstereo;
	if ( m_bStereo ) {
		if ( eyesep > .0 ) {
...
void CGLGadget::display()
{
	...
	if ( !m_bStereo ) {
...
#if 0
		GLfloat wid, wf = 5.0;
		wid = m_stereoinfo.fixpoint * (1.0 / wf);

		m_stereoinfo.left = -wf * ((wid - 0.5 * m_stereoinfo.eyesep) / m_stereoinfo.fixpoint);
		m_stereoinfo.right = wf * ((wid + 0.5 * m_stereoinfo.eyesep) / m_stereoinfo.fixpoint);
		m_stereoinfo.asp = (GLfloat) m_height / (GLfloat) m_width;

		/// begin rendering stereoscopic geometries
		//
		/* First left eye.  */
		...
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glFrustum(m_stereoinfo.left, m_stereoinfo.right, -m_stereoinfo.asp, m_stereoinfo.asp, mindist/20.0, maxdist*4.0);

		glMatrixMode(GL_MODELVIEW);

		glPushMatrix();
		glLoadIdentity();
		gluLookAt(0, 0, (mindist/20.0+maxdist*4.0)/2.0, 0, 0, 0, 0, 1, 0);
		glTranslated( +0.5 * m_stereoinfo.eyesep, 0.0, 0.0);
		glMultMatrixf(mvmat);
		draw();
		glPopMatrix();
		...
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glFrustum(-m_stereoinfo.right, -m_stereoinfo.left, -m_stereoinfo.asp, m_stereoinfo.asp, mindist/20.0, maxdist*4.0);
		...
		double near = mindist/20.0/2.0, far = maxdist*4.0;
...
void CGLTextbox::draw()
{
	...
	for (size_t i = 0; i < m_lines.size(); ++i) {
		...
		/*
		printTextIn3D (m_lines[i].c_str(), x, y, 0,
...
int CRectColorMap::_calcRectGeometry()
{
	...
	char mark[64];
	mark [ sprintf(mark, "%6.1f", m_tickmax) ] = 0;
	GLint tickmarkwidth = glutBitmapLength(m_font, (const unsigned char*)mark) * 3;
	tickmarkwidth += ( m_ticklen*16 + m_markoffset );

	m_sz = 2*(int)(d+3)*10 * (int)((d+1)*2-tickmarkwidth)*10 * 6;
	...
	for (GLint i=0; i<(int)((d+1)*2-tickmarkwidth)*10; i++) {
		...
		y = -d;
		tickv = m_tickmin;
		for (GLint j=0; j<=(int)(d+2)*2*10; j++) {
			if ( fabs(y) <= 1e-6 ) {
				y = .0;
			}
			if ( fabs( y - d ) <= 1e-6 ) {
...
void CRectColorMap::_drawticks()
{
	...
	char mark[64];
	mark [ sprintf(mark, "%6.1f", m_tickmax) ] = 0;
	GLint markwidth = glutBitmapLength(m_font, (const unsigned char*)mark) * 3;
	...
	for (GLint i = 0; i <= m_ticknum; i++) {
		...
		// draw the mark
		mark[ sprintf(mark, "%6.1f", tickv) ] = '\0';
		printTextIn3D(mark, x + m_ticklen + m_markoffset, y, 0, 
...
///////////////////////////////////////////////////////////////////////////////
//
// implemenation of the COptionPanel class
//
...
void COptionPanel::draw()
{
	//std::cout << m_dx << "," << m_dy << "," << m_dz << "\n";
	/*
	GLdouble d = min(m_dx, m_dy);
	glPushAttrib(GL_ALL_ATTRIB_BITS);
	glColor3f(0.3f, .4f, .4f);
	*/
	//glTranslatef(0, -m_dy+40, 0);
	glTranslatef(-m_dx+40, 0, 0);
	/*
...
void COptionPanel::display()
{
	if ( int( m_buttons.size() ) < 1 ) return;
	// set the desirable viewport
	GLint viewport[4];
	glGetIntegerv(GL_VIEWPORT, viewport);
	setParams(true, 0, 0, 
...
void COptionPanel::addButton(const string& btntext, const char* fnIconImage,
							const string& btntext2, const char* fnIconImage2)
{
	...
	if ( NULL == ( btn.pIconImgData = LoadDIBitmap(fnIconImage, &pBmpInfo) ) ) {
		std::cerr << "Failed to read image for button icon - " << 
	...
	if ( strlen(fnIconImage2) >= 1 ) {
		if ( NULL == ( btn.pIconImgData2 = LoadDIBitmap(fnIconImage2, &pBmpInfo) ) ) {
			std::cerr << "Failed to read secondary image for button icon - " << 
...
int COptionPanel::getSelectedBtn(GLdouble x, GLdouble y)
{
	int ret = -1;
	if ( m_buttons.size() < 1 ) {
		return ret;
...
{
	int signx = 1, signy = 1;
	GLdouble startx, starty, twidth, theight, __z = 0;
	...
	for (int i=0; i < szbtn; i++) {
		curW = m_buttons[i].text.length()*3;
		if ( curW > maxW ) {
			maxW = curW;
		}
	}

	int btnW = 33+spacing, btnH = 6+spacing;

	/*
	startx = -m_dx + ( twidth - btnW*szbtn - spacing*(szbtn-1) )/2.0;
	starty = -d/6.0+btnH;
	*/
	startx = max(m_dx,m_dy)*2 + 40 - btnW - spacing;
	starty = m_dy - ( theight - btnH*szbtn - spacing*2*(szbtn-1) )/2.0;
	...
	for (int i = 0; i <= numRow; ++i) {
		...
		// the last button, the "Next" button is smaller and much lower than others
		if ( i == numRow - 1 ) {
			curlby -= btnH*3;
			curlbx += spacing;
			btnH -= spacing;
		...
		for (int j = 0; j < numPerRow; ++j) {
			currtx = curlbx + btnW*signx - spacing;
			currty = curlby + btnH*signy;

			_coordlb.update( curlbx, curlby, __z );
			_coordrt.update( currtx, currty, __z );
			for ( int k = 0; k < 3; ++k ) {
				(m_buttons)[ ibtn ].leftbottom[ pointidxs[k] ] = _coordlb[k]; 
				(m_buttons)[ ibtn ].righttop[ pointidxs[k] ] = _coordrt[k];
				//(m_buttons)[ ibtn ].textpos [ pointidxs[k] ] = (_coordlb[k] + _coordrt[k] )/2.0;
			}
			curW = m_buttons[ibtn].text.length()*3;
			(m_buttons)[ ibtn ].textpos.update( curlbx + spacing + (btnW - curW)/3.0, curlby+spacing, __z );
