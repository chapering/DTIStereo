...
CSitubeRender::CSitubeRender(int argc, char **argv) : CGLIBoxApp(argc, argv),
	...
	m_colorschemeIdx(CLSCHM_ALL_SYMMETRIC),
	...
	m_bBrushEffect(true),
	m_strfnFiberIdx(""),
	m_pressBin(0x07), // to force ROI saving, initialize it to 0
	m_bHaloEffect(false),
	m_fAlpha(0.3),
	m_bSeparateFlagUpdate(false)
{
	for (int i=0;i<3;++i) {
	...
	addOption('f', true, "geometry-model", "the name of source file"
			" containing geometry in the format of tgdata");
#ifdef DWI_EMBEDDING
...
void CSitubeRender::buildTubefromLine(unsigned long lineIdx) 
{
	...
	vector<GLfloat>		tube_ctxcolors;
	vector<GLfloat>		tube_ctxvertices;
	...
	tube_ctxvertices.resize ( szPts * m_lod * 3 );
	tube_ctxcolors.resize ( szPts * m_lod * 4 );
	...
	for (unsigned long idx1 = 0, idx2 = 1; idx1 < szPts; idx1++) {
		...
		for (GLubyte l = 0; l < m_lod; ++l) {
			...
			tube_encodedcolors [ idx1*4 * m_lod + 4*l + 0 ] = 1.0;
			tube_encodedcolors [ idx1*4 * m_lod + 4*l + 1 ] = 1.0;
			tube_encodedcolors [ idx1*4 * m_lod + 4*l + 2 ] = 0.0;
			tube_encodedcolors [ idx1*4 * m_lod + 4*l + 3 ] = 1.0;

			tube_ctxvertices[ idx1*3 * m_lod + 3*l + 0 ] = scaleFactor*0.6*nx + line[idx1*6+3];
			tube_ctxvertices[ idx1*3 * m_lod + 3*l + 1 ] = scaleFactor*0.6*ny + line[idx1*6+4];
			tube_ctxvertices[ idx1*3 * m_lod + 3*l + 2 ] = scaleFactor*0.6*nz + line[idx1*6+5];
			tube_ctxcolors [ idx1*4 * m_lod + 4*l + 0 ] = r;
			tube_ctxcolors [ idx1*4 * m_lod + 4*l + 1 ] = g;
			tube_ctxcolors [ idx1*4 * m_lod + 4*l + 2 ] = b;
			tube_ctxcolors [ idx1*4 * m_lod + 4*l + 3 ] = (bKeyFiber?1.0:m_fAlpha);
	...
	m_ctxtubevertices[ lineIdx ] = tube_ctxvertices;
	m_ctxtubecolors[ lineIdx ] = tube_ctxcolors;
}
...
void CSitubeRender::glInit(void) 
{
	...
	GLfloat d = 1.0, e = 1.01, f = 0.05;

	glLightfv(GL_LIGHT0, GL_CONSTANT_ATTENUATION, &d);
	glLightfv(GL_LIGHT0, GL_LINEAR_ATTENUATION, &e);
	glLightfv(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, &f);
...
void CSitubeRender::keyResponse(unsigned char key, int x, int y) 
{
	...
	{
		char keys[] = {'j', 'h', 'i', 'l', 'k', ','};
		if ( std::find(keys, keys + ARRAY_SIZE(keys), key ) != 
...
void CSitubeRender::specialResponse(int key, int x, int y)
{
	...
	switch (key) {
		case GLUT_KEY_F5:
			...
			return;
		case GLUT_KEY_INSERT:// terminate all siblings as well
			if ( m_pressBin != 0x07 ) {
				m_cout << "ROI not saved, can not quit now...\n";
				return;
			}
			...
			cleanup();
			if ( m_nSiblings >= 1 ) {
#ifndef __MAC  // OK. like it or not, MAC does not support kill
...
void CSitubeRender::mouseMotionResponse(int x, int y)
{
	...
	// update all sorts of Flags only when there is some box operations, as
	// intends to improve the overall performance, esp. when there is no box
	// operation but only global transformation or window update
	// ------- However, this made it quite slow to filter the fiber - the
	// process of filtering was greatly slowed down, although global
	// transformation was speeded up conspicuously
	if ( m_bSeparateFlagUpdate && -1 != m_curselboxidx ) {
		_updateAssortedFlags();
	}
...
int CSitubeRender::mainstay() 
{
	...
	addButton("n", "bicons/6.bmp","s"); // show context or no context
#ifdef DWI_EMBEDDING
	addButton("d", "bicons/7.bmp","x"); 
	addButton(">"); 
	addButton("<"); 
#endif
	addButton("r", "bicons/6.bmp","b"); // brushing or non-brushing
	...
	setPrjInfo(25.0f, 
			...
			(ABS(m_maxCoord[2]) + ABS(m_minCoord[2]))*8.0);
...
void CSitubeRender::do_draw() 
{
	...
	*/
	unsigned long szTotal = m_loader.getSize();
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		if ( !m_bSeparateFlagUpdate ) {
			m_edgeflags[idx] = isLineInSelBox<GLfloat>( 
	...
	// show streamtube model as context for selecting ROIs
	if ( m_bShowContext ) {
		drawTubeContext();
		// show halo effect if commanded
		if ( m_bHaloEffect ) {
			drawHaloes();
		}
	}
	...
	if ( m_bShowContext && "" != m_strfnskeleton ) {
	 ...
	 * here at this point it should be the best place where we can say that one more
...
void CSitubeRender::onIdle(void)
{
	if ( m_nSiblings < 1 ) {
		...
		usleep(10000);
		CSitubeRender::keyResponse(keys[idx], 0, 0);
		angle ++;
		if ( angle >= 360 ) {
			angle = 0;
			idx ++;
...
void CSitubeRender::onReshape( GLsizei w, GLsizei h )
{
...
int CSitubeRender::dumpFiberIdx(const char* fnFiberidx, const char* firstline)
{
...
int CSitubeRender::dumpBoxpos(int bidx, const char* fnBoxpos, const char* firstline)
{
	if ( m_boxes.size() < 1 ) {
		return -1;
	}
	...
	prefix << "tumorbox_" << bidx;
	if ( "" == strfn ) {
		_formFilename(strfn, prefix.str(), m_strfnsrc, false);
	}

	ofstream ofs(strfn.c_str());
	if ( strlen (firstline) < 1 ) {
		ofs << (m_boxes[bidx].isRemovalBased()?"Active":"Exclusive") << "\n";
	}
	else {
		ofs << firstline << "\n";
	}
	m_boxes[bidx].reportself( ofs, true );
	ofs.close();

	m_cout << "box " << bidx << "'s measure dumped into " << strfn << ".\n";
	return 0;
}
...
void CSitubeRender::handleBtnEvents(int event)
{
	switch (event) {
		...
		case 6:
			...
			m_buttons[6].switchStatus();
			break;
		case 7:
			_realkeyResponse('n',0,0);
			break;
		case 8:
			_realkeyResponse('p',0,0);
			break;
#endif
#ifdef DWI_EMBEDDING
		case 9:
			m_buttons[9].switchStatus();
#else
		case 6:
			m_buttons[6].switchStatus();
#endif
			m_bBrushEffect = !m_bBrushEffect;
			break;
		default:
			break;
	}
...
void CSitubeRender::drawContext()
{
	glPushAttrib(GL_ALL_ATTRIB_BITS);
	glPushClientAttrib(GL_ALL_CLIENT_ATTRIB_BITS);
	glPushMatrix();
	...
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glColor4f(0.8,0.8,0.8, m_fAlpha);
...
void CSitubeRender::drawTubeContext()
{
	glPushAttrib(GL_ALL_ATTRIB_BITS);
	glPushClientAttrib(GL_ALL_CLIENT_ATTRIB_BITS);
	...
	// always render the streamtube geometry as the context for selection
	unsigned long szTotal = m_loader.getSize();
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		if ( !m_brushedoutflags[idx] ) {
			continue;
		}

		if ( !m_bSeparateFlagUpdate ) {
			if ( !isLineInRemBox<GLfloat>( 
					&m_ctxtubevertices[idx][0], 
					m_ctxtubevertices[idx].size(),
					6, 3, 
					-(GLfloat)( m_minCoord[0] + m_maxCoord[0] )/2,
					-(GLfloat)( m_minCoord[1] + m_maxCoord[1] )/2,
					-(GLfloat)( m_minCoord[2] + m_maxCoord[2] )/2,
					true) ) {
				m_ctxedgeflags[idx] = GL_FALSE;
				if ( m_bBrushEffect ) {
					m_brushedoutflags[idx] = GL_FALSE;
				}
			}
			else {
				if ( !m_bBrushEffect ) {
					m_ctxedgeflags[idx] = GL_TRUE;
				}
			}
		}

		if ( !m_ctxedgeflags[idx] ) {
			continue;
		}

		glVertexPointer(3, GL_FLOAT, 0, &m_ctxtubevertices[idx][0]);

		if ( m_bUseDirectionColor ) {
			glColorPointer(4, GL_FLOAT, 0, &m_ctxtubecolors[idx][0]);
		}
		else if ( m_bUseOrgColor ) {
			glColorPointer(3, GL_FLOAT, 0, &m_alltubecolors[idx][0]);
		}
		...
		if ( m_bVnormal ) {
			glNormalPointer(GL_FLOAT, 0 ,&m_ctxtubevertices[idx][0]);
		}
		else {
			glNormalPointer(GL_FLOAT, 0 ,&m_alltubenormals[idx][0]);
		}
...
int CSitubeRender::projectSkeleton(int x, int y, bool bZ)
{
	...
	if ( -1 == x && -1 == y ) {
		...
		// initially all skeletonic fibers should all be shown
		m_skeletonEdgeflags.resize( szSkeletonTotal );
		m_skeletonEdgeflags.assign( szSkeletonTotal, GL_TRUE );
		m_brushedoutSkeletonflags.resize ( szSkeletonTotal );
		m_brushedoutSkeletonflags.assign ( szSkeletonTotal, GL_TRUE );
...
void CSitubeRender::drawSkeleton()
{
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		const vector<GLfloat>& line = m_staticSkeleton.getElement(idx);

		if ( !m_brushedoutSkeletonflags[idx] ) {
			continue;
		}
		...
		if ( !m_bSeparateFlagUpdate ) {
			if ( !isLineInRemBox<GLfloat>( 
					...
					-(GLfloat)( m_minCoord[2] + m_maxCoord[2] )/2,
					true) ) {
				m_skeletonEdgeflags[idx] = GL_FALSE;
				if ( m_bBrushEffect ) {
					m_brushedoutSkeletonflags[idx] = GL_FALSE;
				}
			}
			else {
				if ( !m_bBrushEffect ) {
					m_skeletonEdgeflags[idx] = GL_TRUE;
				}
			}
		}

		if ( !m_skeletonEdgeflags[idx] ) {
			continue;
		}
...
void CSitubeRender::_realkeyResponse(unsigned char key, int x, int y)
{
	switch (key) {
		...
		case 'g': // generate different colors
			...
			break;
		case 'v': // toggle the use of vertices as normals
			m_bVnormal = ! m_bVnormal;
			break;
		case 'd': // toggle direction-based coloring
			...
			break;
		case 'm': // alternate between coloring schemes
			{
			...
			break;
#ifdef DWI_EMBEDDING
		case 'p': // move to the previous DWI Image
			if ( "" != m_strdwidir && m_bShowDWIImage ) {
				m_dcmexplorer.prev();
			}
			break;
		case 'n': // move to the next DWI Image
			if ( "" != m_strdwidir && m_bShowDWIImage ) {
				m_dcmexplorer.next();
			}
			break;
		case 'o': // toggle DWI image embedding
			m_bShowDWIImage = !m_bShowDWIImage;
		...
		case 32: // Space bar to trigger next task if any
			...
			CGLIBoxApp::keyResponse(key, x, y);
			return;
		case 'O': // toggle halo effect for the streamtube context
			m_bHaloEffect = !m_bHaloEffect;
			break;
		case 'a': // increase alpha
		case 'A': // decrease alpha
			{
				m_fAlpha += 0.1 * ('A'==key?-1:1);
				if ( m_fAlpha < 0.0 ) m_fAlpha = 0.0;
				if ( m_fAlpha > 1.0 ) m_fAlpha = 1.0;
				m_cout << "alpha updated to " << m_fAlpha << "\n";
				_UPDATE_STREAMTUBES;
			}
			break;
		case 'r': // increase tube radius
		case 'R': // decrease tube radius
			{
				m_fRadius += 0.1 * ('R'==key?-1:1);
				m_bVradius = false;
				if ( m_fRadius < 0.1 ) m_fRadius = 0.1;
				m_cout << "tube radius updated to " << m_fRadius << "\n";
				_UPDATE_STREAMTUBES;
			}
			break;
		case 'x': // increase LOD
		case 'X': // decrease LOD
			{
				...
				if ( lod < 2 ) lod = 2;
				m_lod = lod;
				m_cout << "LOD updated to " << (int)m_lod << "\n";
				_UPDATE_STREAMTUBES;
			}
			break;
		default:
		...
		case GLUT_KEY_F11: 
			...
			// during the post-study data analysis
			{
				...
				//if (1) {
					...
					unsigned long szTotal = m_loader.getSize();
					for (unsigned long idx = 0; idx < szTotal; ++idx) {
						if ( m_expectedFiberIndices.find( idx ) != m_expectedFiberIndices.end() ) {
							m_edgeflags[idx] = GL_TRUE;
						}
					}
					*/
...
void CSitubeRender::_updateAssortedFlags()
{
	unsigned long szTotal = m_loader.getSize();
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		// update fiber flags for the primary object
		m_edgeflags[idx] = isLineInSelBox<GLfloat>( 
				&m_alltubevertices[idx][0], 
				m_alltubevertices[idx].size(),
				6, 3, 
		...
		// update streamtube context related fiber flags
		if ( m_bShowContext && m_brushedoutflags[idx] ) {
			if ( !isLineInRemBox<GLfloat>( 
					&m_ctxtubevertices[idx][0], 
					m_ctxtubevertices[idx].size(),
					6, 3, 
	...
	// update skeleton streamline flags
	if ( "" != m_strfnskeleton && m_bShowContext ) {
		unsigned long szTotal = m_skeletonLoader.getSize();
		for (unsigned long idx = 0; idx < szTotal; ++idx) {
			const vector<GLfloat>& line = m_staticSkeleton.getElement(idx);
